...
class PlanTripUseCase {
    ...
    operator fun invoke(originStopId: String, destStopId: String): List<JourneyPlan> {
        val stopToRoutes = buildStopToRoutes()
        val nowSec = nowMidnightSec()

        Log.d("PlanTrip", "invoke: origin=$originStopId dest=$destStopId nowSec=$nowSec")
        Log.d("PlanTrip", "stopToRoutes size=${stopToRoutes.size}")
        Log.d("PlanTrip", "routes at origin: ${stopToRoutes[originStopId]}")
        Log.d("PlanTrip", "routes at dest: ${stopToRoutes[destStopId]}")
        Log.d("PlanTrip", "stopTimesByStop origin size=${GtfsStaticCache.stopTimesByStop[originStopId]?.size}")
        Log.d("PlanTrip", "stopTimesByStop dest size=${GtfsStaticCache.stopTimesByStop[destStopId]?.size}")

        var results = runBfs(originStopId, destStopId, stopToRoutes, nowSec)
        Log.d("PlanTrip", "pass1 results=${results.size}")

        if (results.isEmpty()) {
            results = runBfs(originStopId, destStopId, stopToRoutes, 0L)
            Log.d("PlanTrip", "pass2 results=${results.size}")
        }

        return results
            .distinctBy { plan -> plan.legs.map { "${it.routeId}|${it.boardStopId}|${it.alightStopId}" } }
            .sortedWith(compareBy({ it.transferCount }, { it.legs.firstOrNull()?.departureSec ?: 0L }))
            .take(5)
    }
    ...
    private fun runBfs(
        ...
        while (queue.isNotEmpty() && results.size < 6) {
            ...
            for ((_, pair) in boardingKey) {
                ...
                for (nextSt in subseq) {
                    ...
                    if (prevBest != null && arrSec > prevBest + 900) continue
